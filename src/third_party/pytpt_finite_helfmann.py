...
class tpt:
    ...
    def __init__(self, P, N, ind_A, ind_B,  ind_C, init_dens):
        ...
        self.fx = None      # Flux projection
        self.fy = None      # Flux projection
    ...
    def reac_current(self, xB, yB, N_phys):
        ...
        # # Project effective current on physical bins only
        ...
        # Number of steps
        K = self.N-1
        fx = np.zeros((K, N_phys), dtype=float)
        fy = np.zeros((K, N_phys), dtype=float)
        
        for k in range(K):
            fp_phys = self.eff_current[k, :N_phys, :N_phys]
            for i in range(N_phys):
                ...
                ex /= e
                ey /= e
                fx[k, i] = np.sum(fp_phys[i, :] * ex)
                fy[k, i] = np.sum(fp_phys[i, :] * ey)

        self.fx = fx
        self.fy = fy
        
        return self.current, self.eff_current, fx, fy
    ...
    def origin_likelihood(self, N_phys, nirvana_idx= None):
        """
        Time-integrated backward origin likelihood over the finite-time window.
    
        ft: finite_tpt instance (after ft.density() and ft.backward_transitions()).
        ind_b: target indices (global state indices, may include nirvana -> filtered)
        Returns: origin over physical bins (length N_phys), normalized to sum 1.
        """
        
        if self.P_back is None:
            raise RuntimeError("Call density() and backward_transitions() before origin_likelihood().")
        
        ind_b = self.ind_B
        
        rho = np.zeros(self.S, dtype=float)
        
        # Exclude nirvana from B
        if nirvana_idx is not None:
            ind_b = ind_b[(ind_b >=0) & (ind_b < nirvana_idx)]            
        else:
            ind_b = ind_b[(ind_b >=0) & (ind_b < self.S)]
            
        rho [ind_b] = 1.0 / float(ind_b.size)
        if nirvana_idx is not None:
            rho[nirvana_idx] = 0.0
        
        ind_b = np.asarray(ind_b, dtype=int)
        ind_b = ind_b[(ind_b >= 0) & (ind_b < nirvana_idx)]  # exclude nirvana & invalid
    
        if ind_b.size == 0:
            return np.full(N_phys, np.nan, dtype=float)
        
        # backward mass distribution over states
        rho = np.zeros(self.S, dtype=float)
        
        # Set final rho at final time as uniform over B
        rho[ind_b] = 1.0 / float(ind_b.size)
        
        # Force zero mass in Nirvana at final time
        rho[nirvana_idx] = 0.0
    
        rho_sum = np.zeros(self.S, dtype=float)
        
        # Start at time N-1 and step backward to time 0:
        # rho_{n-1} = rho_n @ P_back(n)
        for n in range(self.N - 1, -1, -1):
            rho_sum += rho
            if n > 0:
                rho = rho @ self.P_back(n)
                
                # Force nirvana to zero
                rho[nirvana_idx] = 0
        
        # Extract only physcial bins
        out = rho_sum[:N_phys].copy()
        
        # Normalize the returned origin map so it sums to 1
        s = out.sum()
        if s > 0:
            out /= s
        else:
    ...
    def fraction_reactive_transport_gate(self, west_idx, east_idx):
        """
        Fraction of total reactive transport leaving A that crosses a gate (west->east).
    
        Args:
            west_idx: 1D array-like of indices (typically physical bins)
            east_idx: 1D array-like of indices (typically physical bins)
    
        Returns:
            R: gate_flux / K_tot
            gate_flux: sum over steps of eff_current[west,east]
            K_tot: total reactive outflow from A over window (sum rate[:,0] over steps)
        """
        if self.eff_current is None:
            raise RuntimeError("Call reac_current() before fraction_reactive_transport_gate().")
    
        if self.rate is None:
            # compute rates if not done yet
            self.transition_rate()
    
        west_idx = np.asarray(west_idx, dtype=int)
        east_idx = np.asarray(east_idx, dtype=int)
    
        # steps are n=0..N-2; last slice is NaN by construction
        eff_steps = self.eff_current[:self.N - 1, :, :]
        rate_steps = self.rate[:self.N - 1, 0]
    
        K_tot = float(np.nansum(rate_steps))
        if (not np.isfinite(K_tot)) or (K_tot <= 0.0):
            return np.nan, 0.0, K_tot
    
        gate_flux = 0.0
        for n in range(self.N - 1):
            gate_flux += float(eff_steps[n][np.ix_(west_idx, east_idx)].sum())
    
        R = gate_flux / K_tot
        return R, gate_flux, K_tot
